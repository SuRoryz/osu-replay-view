...
class SocialOverlay:
    ...
    def draw(self, dt: float) -> None:
        ...
        for user in users:
            ...
            commands.panel(Rect(row_draw_rect.x + 10.0 * density, row_draw_rect.y + 12.0 * density, 8.0 * density, 8.0 * density), radius=4.0 * density, color=(dot_color[0], dot_color[1], dot_color[2], 0.95 * panel_progress), border_color=(0.0, 0.0, 0.0, 0.0), border_width=0.0)
            nickname = self.app.text.truncate(user.nickname, layout.tokens.typography.body_s, row_draw_rect.w - 88.0 * density)
            commands.text(nickname, row_draw_rect.x + 26.0 * density, row_draw_rect.y + 8.0 * density, layout.tokens.typography.body_s, color=colors.text_primary if user.online else colors.text_secondary, alpha=0.94 * panel_progress)
            suffix = "Friend" if user.is_friend else ("Blocked" if user.is_blocked else "")
            if suffix:
                draw_supporting_text(commands, theme, suffix, row_draw_rect.right - 52.0 * density, row_draw_rect.y + 9.0 * density, layout.tokens.typography.caption, alpha=0.78 * panel_progress, tone="muted")
